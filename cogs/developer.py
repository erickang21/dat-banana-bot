...
class Developer(commands.Cog):
    ...
    async def presence(self, ctx, *, message=None):
        ...
        if not message:
            await self.bot.change_presence(activity=discord.Game(name=f"uwu help | {len(self.bot.guilds)} servers"))
            return await ctx.send(f"The bot's status message has been cleared! {self.bot.get_emoji(692476302233567342)}")
        else:
            await self.bot.change_presence(activity=discord.Game(name=f"uwu help | {len(self.bot.guilds)} servers | {message}"))
