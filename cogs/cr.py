...
class CR:
    ...
    async def crprofile(self, ctx, crtag=None):
        ...
        try:
            ...
            em.add_field(name='Wins', value=f"{profile.games.wins} ({win_percent * 100}% of all games) {self.emoji('battle')}")
            ...
            em.add_field(name='Favorite Card', value=f'{profile.stats.favoriteCard.name if hasattr(profile.stats.favoriteCard, "name") else "No favourite card just yet! Play a match to get one."} {self.emoji(profile.stats.favoriteCard or "soon")}')
            if not profile.rank:
                globalrank = 'Unranked'
            else:
