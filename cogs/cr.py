...
class CR:
    ...
    async def crprofile(self, ctx, crtag=None):
        ...
        try:
            ...
            em.add_field(name='Favorite Card', value=f'{profile.stats.favoriteCard.name or "No favourite card just yet! Play a match to get one."} {self.emoji(profile.stats.favoriteCard.name)}')
            if not profile.rank:
                globalrank = 'Unranked'
            else:
