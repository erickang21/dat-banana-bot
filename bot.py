...
{message.content}
    ...
    if level:
        ...
        bal = await balance(message.guild, message.author)
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, missing_param_errors):
        em = discord.Embed(color=discord.Color(value=0xf44242), title="Incorrect!")
    ...
    if isinstance(error, commands.NotOwner):
        ...
        em = discord.Embed(color=ctx.author.color, title=f"{bot.get_emoji(704757111493754880)} Error {bot.get_emoji(704757111493754880)}")
        desc = ""
